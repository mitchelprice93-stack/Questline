...
-- skip past debuff reconciliation entirely (no row in modifiers → no
-- debuff applied → free completion). complete_quest now reconciles
-- debuffs at the top, before reading the modifier table.
...
begin
  ...
  -- Reconcile any pending debuffs (deadline-miss, etc.) before computing
  -- the modifier sum. Without this, completing an overdue quest without
  -- visiting the Character Sheet first would skip past the debuff entirely.
