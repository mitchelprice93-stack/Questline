...
-- buffs still stack as before, so chasing variety still grows the
-- modifier sum.
...
begin
  ...
  -- expires_at instead of creating a duplicate row, so chasing the same
  -- buff repeatedly extends its lifetime rather than stacking the modifier.
  if v_granted_name is not null and v_granted_pct is not null and v_granted_cond is not null then
    ...
    if v_condition_met then
      ...
      -- Look up an active buff with the same name (case-insensitive) for
      -- this user. The lookup runs after the consume step above, so an
      -- expired same-name buff doesn't count.
      ...
      if v_existing_buff_id is not null then
        -- Refresh: take the later of the two expiries so the player never
        -- loses time by re-earning the same buff with a shorter source.
        update public.modifiers
